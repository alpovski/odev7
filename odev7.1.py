# ödev 7 with list into dict style
...
def main():
    ...
    for sporcu_no in range(sporcu_say):
        puanList = [0] * KAC_ADET_PUAN_TURU_VAR
        sporcuDict[sporcu_no] = puanList
        for atis_no in range(1, atis_say + 1):
            try:
                ...
                atis = int(input(f"{sporcu_no + 1}. sporcunun {atis_no}. atışını giriniz(0-10): "))
            sporcuDict[sporcu_no][KAC_ADET_PUAN_TURU_VAR-atis-1] += 1
            if atis == 0:
    ...
    for key, val in sporcuDict.items():
        print(key, val)
